package cn.zx.website.algorithms.graph;
	...
	public static Map<Graph.Vertex<Integer>, Map<Graph.Vertex<Integer>, Set<Graph.Edge<Integer>>>> getAllPairsShortestPaths(
		...
		for (Graph.Vertex<Integer> v : g.getVerticies()) {
			Graph<Integer> graph = new Graph<Integer>(g); // Clone the original
															// graph
			Graph.Vertex<Integer> connector = new Graph.Vertex<Integer>(
					Integer.MAX_VALUE); // Make
										// new
										// Vertex
										// that
										// connects
			...
			Graph.Edge<Integer> e = new Graph.Edge<Integer>(0, connector, graph
					.getVerticies().get(indexOfV));
			connector.addEdge(e);
			graph.getEdges().add(e);

			Map<Graph.Vertex<Integer>, Graph.CostPathPair<Integer>> costs = BellmanFord
					.getShortestPaths(graph, connector);
			if (BellmanFord.containsNegativeWeightCycle()) {
				System.out
						.println("Graph contains a negative weight cycle. Cannot compute shortest path.");
				return null;
			}
			for (Graph.Vertex<Integer> v2 : costs.keySet()) {
				int index = graph.getVerticies().indexOf(v2);
				Graph.Vertex<Integer> vertexToAdjust = graph.getVerticies()
						.get(index);
			...
			Map<Graph.Vertex<Integer>, Graph.CostPathPair<Integer>> costPaths = Dijkstra
					.getShortestPaths(graph, v);
