...
public class Treap<T extends Comparable<T>> extends BinarySearchTree<T>
	...
	private static int randomSeed = Integer.MAX_VALUE; // This should be at
														// least twice the
														// number of Nodes
	...
	private void heapify(TreapNode<T> current) {
		...
		while (parent != null && current.priority > parent.priority) {
			...
			if (parent.lesser != null && parent.lesser.equals(current)) {
				...
				System.err
						.println("YIKES! Parent should have at least one non-NULL child which should be me.");
			}
	...
	private static class TreapNode<T extends Comparable<T>> extends Node<T> {
		...
		public String toString() {
			...
			builder.append("priorty=").append(priority).append(" value=")
					.append(id);
			if (parent != null)
			...
			if (greater != null)
				builder.append("right=").append(greater.toString())
						.append("\n");
	...
	protected static class TreapPrinter {
		...
		private static <T extends Comparable<T>> String getString(
				TreapNode<T> node, String prefix, boolean isTail) {
			StringBuilder builder = new StringBuilder();
			...
			if (children != null) {
				for (int i = 0; i < children.size() - 1; i++) {
					...
					builder.append(getString(treapNode, prefix
							+ (isTail ? "    " : "│   "), false));
				}
				if (children.size() >= 1) {
					TreapNode<T> treapNode = (TreapNode<T>) children
							.get(children.size() - 1);
					builder.append(getString(treapNode, prefix
							+ (isTail ? "    " : "│   "), true));
				}
