...
public interface Queue<T> extends IQueue<T> {
	...
	public static class ArrayQueue<T> implements Queue<T> {
		...
		public boolean offer(T value) {
			...
			if (currentSize >= array.length) {
				...
				// Since the array can wrap around, make sure you grab the first
				// chunk
				int adjLast = lastIndex % array.length;
				if (adjLast <= firstIndex) {
					System.arraycopy(array, 0, temp, array.length - adjLast,
							adjLast + 1);
				}
				System.arraycopy(array, firstIndex, temp, 0, array.length
						- firstIndex);
				array = temp;
		...
		public T poll() {
			...
			if (shrinkSize >= MINIMUM_SIZE && (array.length > shrinkSize)) {
				...
				// chunk
				int adjLast = lastIndex % array.length;
				int endIndex = (lastIndex > array.length) ? array.length
						: lastIndex;
				if (adjLast <= firstIndex) {
					System.arraycopy(array, 0, temp, array.length - firstIndex,
							adjLast);
				}
				System.arraycopy(array, firstIndex, temp, 0, endIndex
						- firstIndex);
				array = temp;
				lastIndex = (lastIndex - firstIndex);
				firstIndex = 0;
		...
		private static class Node<T> {
			...
			public String toString() {
				return "value=" + value + " previous="
						+ ((prev != null) ? prev.value : "NULL") + " next="
						+ ((next != null) ? next.value : "NULL");
			}
		}
	}

	public static class JavaCompatibleArrayQueue<T> extends
		...
		private static class ArrayQueueIterator<T> implements
	...
	public static class JavaCompatibleLinkedQueue<T> extends
			java.util.AbstractQueue<T> {
		...
		private static class LinkedQueueIterator<T> implements
				java.util.Iterator<T> {
