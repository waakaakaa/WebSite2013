...
public class BTree<T extends Comparable<T>> implements ITree<T> {
	...
	private boolean combined(Node<T> node) {
		...
		if (rightNeighbor != null && rightNeighborSize > minKeySize) {
			...
			if (leftNeighbor != null && leftNeighborSize > minKeySize) {
				...
				T removeValue = leftNeighbor
						.getKey(leftNeighbor.numberOfKeys() - 1);
				int prev = getIndexOfNextValue(parent, removeValue);
				T parentValue = parent.removeKey(prev);
				T neighborValue = leftNeighbor.removeKey(leftNeighbor
						.numberOfKeys() - 1);
				node.addKey(parentValue);
				parent.addKey(neighborValue);
				if (leftNeighbor.numberOfChildren() > 0) {
					node.addChild(leftNeighbor.removeChild(leftNeighbor
							.numberOfChildren() - 1));
				}
	...
	private static class Node<T extends Comparable<T>> {
		...
		public String toString() {
			...
			if (children != null) {
				builder.append("keySize=").append(numberOfKeys())
						.append(" children=").append(numberOfChildren())
						.append("\n");
			}
	...
	public static class JavaCompatibleBTree<T extends Comparable<T>> extends
		...
		private static class BTreeIterator<C extends Comparable<C>> implements
