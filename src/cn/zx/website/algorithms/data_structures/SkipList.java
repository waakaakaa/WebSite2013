...
public class SkipList<T extends Comparable<T>> implements IList<T> {
	...
	public boolean remove(T value) {
		...
		if (oldNode.isAnOffRamp)
			refactorExpressLanes();
	...
	public String toString() {
		...
		if (aNode != null) {
			...
			while (aNode != null) {
				builder.append(aNode.getIndex()).append("=[")
						.append(aNode.getValue()).append(",")
						.append(aNode.isAnOffRamp).append("]");
				aNode = aNode.next;
				if (aNode != null)
					builder.append(", ");
			}
	...
	private static class ExpressNode<T extends Comparable<T>> extends Node<T> {
		...
		public String toString() {
			...
			if (next != null && (next instanceof ExpressNode)) {
				...
				builder.append(" pointer=[").append(pointerRoot.value)
						.append("]");
			} else {
				builder.append("index=").append(getIndex());
				builder.append(" value=").append(getValue());
				builder.append(" width=").append(width);
				if (next != null)
					builder.append(" node=[").append(next.value).append("=")
							.append(next.isAnOffRamp).append("]");
			}
			return builder.toString();
		}
	}
	...
	public static class JavaCompatibleSkipList<T extends Comparable<T>> extends
		...
		private static class SkipListListIterator<T extends Comparable<T>>
