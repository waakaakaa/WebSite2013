...
public class BinarySearchTree<T extends Comparable<T>> implements ITree<T> {
	 ...
	 *            can be NULL.
	 */
	protected void replaceNodeWithNode(Node<T> nodeToRemoved,
		...
		if (replacementNode != null) {
			...
			if (nodeToRemoveLesser != null
					&& !nodeToRemoveLesser.equals(replacementNode)) {
			...
			if (nodeToRemoveGreater != null
					&& !nodeToRemoveGreater.equals(replacementNode)) {
			...
			if (replacementParent != null
					&& !replacementParent.equals(nodeToRemoved)) {
				Node<T> replacementParentLesser = replacementParent.lesser;
				Node<T> replacementParentGreater = replacementParent.greater;
				if (replacementParentLesser != null
						&& replacementParentLesser.equals(replacementNode)) {
				...
				} else if (replacementParentGreater != null
						&& replacementParentGreater.equals(replacementNode)) {
		...
		if (parent == null) {
			...
			if (root != null)
				...
				&& (parent.lesser.id.compareTo(nodeToRemoved.id) == 0)) {
			parent.lesser = replacementNode;
			if (replacementNode != null)
				replacementNode.parent = parent;
		} else if (parent.greater != null
				&& (parent.greater.id.compareTo(nodeToRemoved.id) == 0)) {
	...
	public T[] getDFS(DepthFirstSearchOrder order) {
		...
		while (index < size && node != null) {
			Node<T> parent = node.parent;
			Node<T> lesser = (node.lesser != null && !added
					.contains(node.lesser)) ? node.lesser : null;
			Node<T> greater = (node.greater != null && !added
					.contains(node.greater)) ? node.greater : null;
	...
	private static class JavaCompatibleBinarySearchTree<T extends Comparable<T>>
		...
		private static class BinarySearchTreeIterator<C extends Comparable<C>>
