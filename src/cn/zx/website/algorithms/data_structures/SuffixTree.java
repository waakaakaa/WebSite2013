...
public class SuffixTree<C extends CharSequence> {
	...
	public String getEdgesTable() {
		...
		if (edgeMap.size() > 0) {
			...
			for (int key : edgeMap.keySet()) {
				...
				builder.append("\t" + e.startNode + "\t" + e.endNode + "\t"
						+ suffix + "\t" + e.firstCharIndex + "\t"
						+ e.lastCharIndex + "\t");
				int begin = e.firstCharIndex;
				int end = (lastCharIndex < e.lastCharIndex) ? lastCharIndex
						: e.lastCharIndex;
	...
	private void addPrefix(int index) {
		...
		while (true) {
			...
			if (isExplicit()) {
				...
				parentNodeIndex = edge.split(currentNode, firstCharIndex,
						lastCharIndex);
			}
			edge = new Edge<C>(this, index, characters.length - 1,
					parentNodeIndex);
	...
	private void canonize() {
		...
		while (edgeSpan <= (lastCharIndex - firstCharIndex)) {
			...
			if (firstCharIndex <= lastCharIndex) {
				edge = Edge
						.find(this, edge.endNode, characters[firstCharIndex]);
	...
	public String toString() {
		...
		builder.append("End of word character = ").append(END_SEQ_CHAR)
				.append("\n");
	...
	private static class Edge<C extends CharSequence> implements
			Comparable<Edge<C>> {

		private static final int KEY_MOD = 2179; // Should be a prime that is
													// roughly 10% larger than
													// the
													// String
		...
		private static <C extends CharSequence> Edge<C> find(
				SuffixTree<C> tree, int node, char c) {
			int key = key(node, c);
		...
		private int split(int originNode, int firstCharIndex, int lastCharIndex) {
			remove(this);
			Edge<C> newEdge = new Edge<C>(tree, this.firstCharIndex,
					this.firstCharIndex + lastCharIndex - firstCharIndex,
					originNode);
		...
		public boolean equals(Object obj) {
			...
			Edge<C> e = (Edge<C>) obj;
			if (startNode == e.startNode
					&& tree.characters[firstCharIndex] == tree.characters[e.firstCharIndex]) {
				return true;
			}
		...
		public String toString() {
			...
			builder.append("firstCharIndex=").append(firstCharIndex)
					.append("\n");
	...
	protected static class TreePrinter {
		...
		public static <C extends CharSequence> String getString(
				SuffixTree<C> tree) {
			return getString(tree, null, "", true);
		}

		private static <C extends CharSequence> String getString(
				SuffixTree<C> tree, Edge<C> e, String prefix, boolean isTail) {
			StringBuilder builder = new StringBuilder();
			int value = 0;
			if (e != null) {
				value = e.endNode;
				String string = tree.string.substring(e.firstCharIndex,
						e.lastCharIndex + 1);
			...
			if (tree.edgeMap.size() > 0) {
				...
				if (children.size() > 0) {
					for (int i = 0; i < children.size() - 1; i++) {
						...
						builder.append(getString(tree, edge, prefix
								+ (isTail ? "    " : "│   "), false));
					}
					if (children.size() >= 1) {
