package cn.zx.website.algorithms.data_structures;
...
public abstract class QuadTree<G extends QuadTree.GeometricObject> {
	 ...
	 */
	public abstract List<G> queryRange(float x, float y, float width,
			float height);
	...
	/**
	 * A PR (Point Region) Quadtree is a four-way search trie. This means that
	 * each node has either four (internal guide node) or zero (leaf node)
	 * children. Keys are only stored in the leaf nodes, all internal nodes act
	 * as guides towards the keys.
	 * 
	 * This implementation is a PR QuadTree which uses "Buckets" to prevent
	 * stalky trees.
	 */
	public static class PointRegionQuadTree<P extends QuadTree.XYPoint> extends
			QuadTree<P> {

		private PointRegionQuadNode<P> root = null;

		/**
		 * Create a quadtree who's upper left coordinate is located at x,y and
		 * it's bounding box is described by the height and width. This uses a
		 * default leafCapacity of 4 and a maxTreeHeight of 20.
		 * 
		 * @param x
		 ...
		 * @param width
		 *            Width of the bounding box containing all points
		 * @param height
		...
		/**
		 * Create a quadtree who's upper left coordinate is located at x,y and
		 * it's bounding box is described by the height and width.
		 * 
		 * @param x
		 *            Upper left X coordinate
		 * @param y
		 ...
		 *            Height of the bounding box containing all points
		 * @param leafCapacity
		 *            Max capacity of leaf nodes. (Note: All data is stored in
		 *            leaf nodes)
		 */
		public PointRegionQuadTree(float x, float y, float width, float height,
				int leafCapacity) {
		 ...
		 *            leaf nodes)
		 * @param maxTreeHeight
		 *            Max height of the quadtree. (Note: If this is defined, the
		 *            tree will ignore the max capacity defined by leafCapacity)
		 */
		public PointRegionQuadTree(float x, float y, float width, float height,
				int leafCapacity, int maxTreeHeight) {
			XYPoint xyPoint = new XYPoint(x, y);
			AxisAlignedBoundingBox aabb = new AxisAlignedBoundingBox(xyPoint,
					width, height);
		...
		public List<P> queryRange(float x, float y, float width, float height) {
			...
			XYPoint xyPoint = new XYPoint(x, y);
			AxisAlignedBoundingBox range = new AxisAlignedBoundingBox(xyPoint,
					width, height);
			root.queryRange(range, pointsInRange);
			return (List<P>) pointsInRange;
		}

		protected static class PointRegionQuadNode<XY extends QuadTree.XYPoint>
				extends QuadNode<XY> {

			// max number of children before sub-dividing
			protected static int maxCapacity = 0;
			// max height of the tree (will over-ride maxCapacity when
			// height==maxHeight)
			 ...
			 * {@inheritDoc}
			 * 
			 * returns True if inserted. returns False if not in bounds of tree
			 * OR tree already contains point.
			 */
			@Override
			protected boolean insert(XY p) {
				...
				if ((height == maxHeight)
						|| (isLeaf() && points.size() < maxCapacity)) {
					points.add(p);
					return true;
				}
				...
				// Otherwise, the point cannot be inserted for some unknown
				// reason (which should never happen)
				return false;
			}
			 ...
			 * {@inheritDoc}
			 * 
			 * This method will merge children into self if it can without
			 * overflowing the maxCapacity param.
			 */
			@Override
			...
			private void subdivide() {
				...
				AxisAlignedBoundingBox aabbNW = new AxisAlignedBoundingBox(
						aabb.upperLeft, w, h);
				northWest = new PointRegionQuadNode<XY>(aabbNW);
				((PointRegionQuadNode<XY>) northWest).height = height + 1;

				XYPoint xyNE = new XYPoint(aabb.upperLeft.x + w,
						aabb.upperLeft.y);
				AxisAlignedBoundingBox aabbNE = new AxisAlignedBoundingBox(
						xyNE, w, h);
				northEast = new PointRegionQuadNode<XY>(aabbNE);
				((PointRegionQuadNode<XY>) northEast).height = height + 1;

				XYPoint xySW = new XYPoint(aabb.upperLeft.x, aabb.upperLeft.y
						+ h);
				AxisAlignedBoundingBox aabbSW = new AxisAlignedBoundingBox(
						xySW, w, h);
				southWest = new PointRegionQuadNode<XY>(aabbSW);
				((PointRegionQuadNode<XY>) southWest).height = height + 1;

				XYPoint xySE = new XYPoint(aabb.upperLeft.x + w,
						aabb.upperLeft.y + h);
				AxisAlignedBoundingBox aabbSE = new AxisAlignedBoundingBox(
						xySE, w, h);
			...
			private void merge() {
				...
				// Children and leafs, see if you can remove point and merge
				// into this node
				...
				if ((size() + total) < maxCapacity) {
					this.points
							.addAll(((PointRegionQuadNode<XY>) northWest).points);
					this.points
							.addAll(((PointRegionQuadNode<XY>) northEast).points);
					this.points
							.addAll(((PointRegionQuadNode<XY>) southWest).points);
					this.points
							.addAll(((PointRegionQuadNode<XY>) southEast).points);
			 ...
			 */
			@Override
			protected void queryRange(AxisAlignedBoundingBox range,
					List<XY> pointsInRange) {
				// Automatically abort if the range does not collide with this
				// quad
	...
	/**
	 * MX-CIF quadtree is a variant of quadtree data structure which supports
	 * area-based query. It is designed for storing a set of rectangles
	 * (axis-aligned bounded box) in a dynamic environment.
	 */
	public static class MxCifQuadTree<B extends QuadTree.AxisAlignedBoundingBox>
			extends QuadTree<B> {
		 ...
		 *            Height of the bounding box containing all points
		 * @param minWidth
		 *            The tree will stop splitting when leaf node's width <=
		 *            minWidth
		 * @param minHeight
		 *            The tree will stop splitting when leaf node's height <=
		 *            minHeight
		 */
		public MxCifQuadTree(float x, float y, float width, float height,
		...
		/**
		 * Insert rectangle whose upper-left point is located at X,Y and has a
		 * height and width into tree.
		 * 
		 * @param x
		 *            X position of upper-left hand corner.
		 * @param y
		...
		/**
		 * Remove rectangle whose upper-left point is located at X,Y and has a
		 * height and width into tree.
		 * 
		 * @param x
		 *            X position of upper-left hand corner.
		...
		protected static class MxCifQuadNode<AABB extends QuadTree.AxisAlignedBoundingBox>
				extends QuadNode<AABB> {
			...
			protected boolean insert(AABB b) {
				...
				// Subdivide then add the objects to whichever node will accept
				// it
				if (isLeaf())
					subdivide(b);
				...
				if (!inserted) {
					// Couldn't insert into children (it could strattle the
					// bounds of the box)
					aabbs.add(b);
					return true;
				} else {
			...
			private boolean insertIntoChildren(AABB b) {
				...
				if (northWest.aabb.insideThis(b) && northWest.insert(b))
					return true;
				if (northEast.aabb.insideThis(b) && northEast.insert(b))
					return true;
				if (southWest.aabb.insideThis(b) && southWest.insert(b))
					return true;
				if (southEast.aabb.insideThis(b) && southEast.insert(b))
					return true;
				return false;
			}
			...
			private boolean removeFromChildren(AABB b) {
				...
				if (northWest.remove(b))
					return true;
				if (northEast.remove(b))
					return true;
				if (southWest.remove(b))
					return true;
				if (southEast.remove(b))
					return true;
				return false; // should never happen
			}
			...
			@Override
			protected void queryRange(AxisAlignedBoundingBox range,
					List<AABB> geometricObjectsInRange) {
				// Automatically abort if the range does not collide with this
				// quad
				if (!aabb.intersectsBox(range))
					return;
	...
	protected static abstract class QuadNode<G extends QuadTree.GeometricObject>
			implements Comparable<QuadNode<G>> {
		 ...
		 * 
		 * @param g
		 *            Geometric object to remove from tree.
		 ...
		 * 
		 * @param range
		 *            Upper-left and width,height of a axis-aligned bounding
		 *            box.
		 * @param geometricObjectsInRange
		 *            Geometric objects inside the bounding box.
		 */
		protected abstract void queryRange(AxisAlignedBoundingBox range,
				List<G> geometricObjectsInRange);
	...
	public static class XYPoint extends GeometricObject implements
			Comparable<XYPoint> {
	...
	public static class AxisAlignedBoundingBox extends GeometricObject
			implements Comparable<AxisAlignedBoundingBox> {
		...
		public AxisAlignedBoundingBox(XYPoint upperLeft, float width,
				float height) {
		...
		public boolean insideThis(AxisAlignedBoundingBox b) {
			if (b.minX >= minX && b.maxX <= maxX && b.minY >= minY
					&& b.maxY <= maxY) {
				// INSIDE
				return true;
			}
	...
	protected static class TreePrinter {
		...
		public static <T extends GeometricObject> String getString(
				QuadTree<T> tree) {
		...
		private static <T extends GeometricObject> String getString(
				QuadNode<T> node, String prefix, boolean isTail) {
			StringBuilder builder = new StringBuilder();

			builder.append(prefix + (isTail ? "└── " : "├── ") + " node={"
					+ node.toString() + "}\n");
			List<QuadNode<T>> children = null;
			if (node.northWest != null || node.northEast != null
					|| node.southWest != null || node.southEast != null) {
