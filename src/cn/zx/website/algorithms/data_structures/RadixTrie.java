...
public class RadixTrie<K extends CharSequence, V> implements
		PatriciaTrie.INodeCreator, IMap<K, V> {
	...
	private boolean validate(PatriciaTrie.Node node, String string,
	 ...
	 */
	@Override
	public PatriciaTrie.Node createNewNode(PatriciaTrie.Node parent,
			char[] seq, boolean type) {
		return (new RadixNode<K, V>(parent, seq, type));
	}

	protected static final class RadixNode<K extends CharSequence, V> extends
			PatriciaTrie.Node implements Comparable<PatriciaTrie.Node> {
		...
		protected RadixNode(PatriciaTrie.Node parent, char[] string,
				boolean type) {
			super(parent, string, type);
		}

		protected RadixNode(PatriciaTrie.Node parent, char[] string,
				boolean type, V value) {
			super(parent, string, type);
			this.value = value;
		}
	...
	protected static class RadixTreePrinter<K extends CharSequence, V> {
		...
		public static <K extends CharSequence, V> String getString(
				RadixTrie<K, V> map) {
			return getString(map.trie.root, "", null, true);
		}

		protected static <K extends CharSequence, V> String getString(
				PatriciaTrie.Node node, String prefix, String previousString,
				boolean isTail) {
			StringBuilder builder = new StringBuilder();
			String string = null;
			if (node.string != null) {
			...
			if (node instanceof RadixNode) {
				...
				builder.append(prefix
						...
						+ ((radix.string != null) ? "("
								...
								+ ") "
								+ "["
								+ ((node.type == PatriciaTrie.WHITE) ? "WHITE"
										: "BLACK")
								+ "] "
								+ string
	...
	private static class JavaCompatibleIteratorWrapper<K extends CharSequence, V>
		...
		public JavaCompatibleIteratorWrapper(RadixTrie<K, V> map,
				java.util.Iterator<java.util.Map.Entry<K, V>> iter) {
			this.map = map;
			this.iter = iter;
		}
	...
	private static class JavaCompatibleMapEntry<K extends CharSequence, V>
	...
	private static class JavaCompatibleRadixTrieMap<K extends CharSequence, V>
		...
		public java.util.Set<java.util.Map.Entry<K, V>> entrySet() {
			java.util.Set<java.util.Map.Entry<K, V>> set = new java.util.HashSet<java.util.Map.Entry<K, V>>() {
				...
				public java.util.Iterator<java.util.Map.Entry<K, V>> iterator() {
					return (new JavaCompatibleIteratorWrapper<K, V>(map,
							super.iterator()));
				}
			};
		...
		private void levelOrder(PatriciaTrie.Node node, String string,
			...
			if (node instanceof RadixNode) {
				...
				if (tmn.type == PatriciaTrie.WHITE) {
					...
					java.util.Map.Entry<K, V> entry = new JavaCompatibleMapEntry<K, V>(
							s, tmn.value);
					set.add(entry);
				}
			}
