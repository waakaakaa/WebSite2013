...
public class Trie<C extends CharSequence> implements ITree<C> {
	...
	protected C remove(Node node) {
		...
		if (node.childrenSize > 0) {
			...
			// represents a string
			while (previous != null && previous.isWord == false
					&& previous.childrenSize == 0) {
	...
	protected static class Node {
		...
		protected boolean isWord = false; // Signifies this node represents a
											// word
		protected char character = SENTINAL; // First character that is
	...
	protected static class TriePrinter {
		...
		protected static <C extends CharSequence> String getString(Node node,
			...
			builder.append(prefix
					+ (isTail ? "└── " : "├── ")
					+ ((node.isWord == true) ? ("(" + node.character + ") " + string)
							: node.character) + "\n");
			if (node.children != null) {
				for (int i = 0; i < node.childrenSize - 1; i++) {
					builder.append(getString(node.children[i], prefix
							+ (isTail ? "    " : "│   "), string, false));
				}
				if (node.childrenSize >= 1) {
					builder.append(getString(
							node.children[node.childrenSize - 1], prefix
									+ (isTail ? "    " : "│   "), string, true));
				}
			}
			return builder.toString();
	...
	public static class JavaCompatibleTrie<C extends CharSequence> extends
		...
		private static class TrieIterator<C extends CharSequence> implements
			...
			private void getNodesWhichRepresentsWords(Trie.Node node,
					String string, java.util.Map<Trie.Node, String> nodesMap) {
				StringBuilder builder = new StringBuilder(string);
				if (node.character != Node.SENTINAL)
					builder.append(node.character);
				if (node.isWord)
