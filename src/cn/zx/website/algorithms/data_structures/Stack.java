...
public interface Stack<T> extends IStack<T> {
	...
	public static class LinkedStack<T> implements Stack<T> {
		...
		private static class Node<T> {
			...
			@Override
			public String toString() {
				return "value=" + value + " above="
						+ ((above != null) ? above.value : "NULL") + " below="
						+ ((below != null) ? below.value : "NULL");
			}
		}
	}

	public static class JavaCompatibleArrayStack<T> extends
		 ...
		 * {@inheritDoc}
		 * 
		 * This iterator is NOT thread safe and is invalid when the data
		...
		private static class ArrayStackIterator<T> implements
	...
	public static class JavaCompatibleLinkedStack<T> extends
			java.util.AbstractQueue<T> {
		...
		private static class LinkedStackIterator<T> implements
				java.util.Iterator<T> {
