...
public class AVLTree<T extends Comparable<T>> extends BinarySearchTree<T>
	...
	protected static class AVLNode<T extends Comparable<T>> extends Node<T> {
		...
		@Override
		public String toString() {
			return "value=" + id + " height=" + height + " parent="
	...
	protected static class AVLTreePrinter {
		...
		private static <T extends Comparable<T>> String getString(
				AVLNode<T> node, String prefix, boolean isTail) {
			...
			if (children != null) {
				for (int i = 0; i < children.size() - 1; i++) {
					builder.append(getString((AVLNode<T>) children.get(i),
							prefix + (isTail ? "    " : "│   "), false));
				}
				if (children.size() >= 1) {
					builder.append(getString(
							(AVLNode<T>) children.get(children.size() - 1),
							prefix + (isTail ? "    " : "│   "), true));
				}
			}
