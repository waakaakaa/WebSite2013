...
public class RedBlackTree<T extends Comparable<T>> extends BinarySearchTree<T>
		implements BinarySearchTree.INodeCreator<T> {
		...
		if (parent.color == RED && uncle.color == RED) {
			...
			if (parent.color == RED && uncle.color == BLACK) {
				...
				if (node.equals(parent.greater)
						&& parent.equals(grandParent.lesser)) {
					// right-left
					rotateLeft(parent);
					...
					uncle = node.getUncle();
				} else if (node.equals(parent.lesser)
						&& parent.equals(grandParent.greater)) {
					// left-right
					rotateRight(parent);
	...
	protected void removeNode(Node<T> node) {
		...
		if (nodeRemoved.isLeaf()) {
			...
			if (lesser.id != null && greater.id != null) {
				// Two children
				RedBlackNode<T> greatestInLesser = (RedBlackNode<T>) this
						.getGreatest(lesser);
	 ...
	 */
	private void replaceValueOnly(RedBlackNode<T> nodeToReplace,
			RedBlackNode<T> nodeToReplaceWith) {
	 ...
	 *            will not be changed.
	 */
	private void replaceWithChild(RedBlackNode<T> nodeToReplace,
			RedBlackNode<T> nodeToReplaceWith) {
		nodeToReplace.id = nodeToReplaceWith.id;
	...
	protected static class RedBlackNode<T extends Comparable<T>> extends
			Node<T> {
		...
		protected RedBlackNode<T> getUncle() {
			...
			} else if (grandParent.greater != null
					&& grandParent.greater.equals(parent)) {
		...
		@Override
		public String toString() {
			return "id=" + id + " color=" + ((color == RED) ? "RED" : "BLACK")
					+ " isLeaf=" + isLeaf() + " parent="
					+ ((parent != null) ? parent.id : "NULL") + " lesser="
					+ ((lesser != null) ? lesser.id : "NULL") + " greater="
	...
	protected static class RedBlackTreePrinter {
		...
		private static <T extends Comparable<T>> String getString(
				RedBlackNode<T> node, String prefix, boolean isTail) {
			...
			builder.append(prefix
					...
					+ node.id
					+ " [parent="
					+ ((node.parent != null) ? node.parent.id : "NULL")
					+ " grand-parent="
					+ ((node.parent != null && node.parent.parent != null) ? node.parent.parent.id
							: "NULL") + "]\n");
			...
			if (children != null) {
				for (int i = 0; i < children.size() - 1; i++) {
					builder.append(getString((RedBlackNode<T>) children.get(i),
							prefix + (isTail ? "    " : "│   "), false));
				}
				if (children.size() >= 1) {
					builder.append(getString(
							(RedBlackNode<T>) children.get(children.size() - 1),
							prefix + (isTail ? "    " : "│   "), true));
				}
			}
	...
	public static class JavaCompatibleRedBlackTree<T extends Comparable<T>>
		...
		private static class RedBlackTreeIterator<C extends Comparable<C>>
