...
import cn.zx.website.algorithms.data_structures.BinarySearchTree.Node;
...
public class TreeMap<K extends Comparable<K>, V> implements
		BinarySearchTree.INodeCreator<K>, IMap<K, V> {
	...
	protected static class TreeMapNode<K extends Comparable<K>, V> extends
			AVLTree.AVLNode<K> {
	...
	protected static class TreeMapPrinter {
		...
		public static <K extends Comparable<K>, V> String getString(
				TreeMap<K, V> map) {
		...
		private static <K extends Comparable<K>, V> String getString(
				TreeMapNode<K, V> node, String prefix, boolean isTail) {
			StringBuilder builder = new StringBuilder();

			builder.append(prefix
					+ (isTail ? "└── " : "├── ")
					+ ((node.id != null) ? (node.id + " = " + node.value)
							: node.id) + "\n");
			...
			if (children != null) {
				for (int i = 0; i < children.size() - 1; i++) {
					builder.append(getString(
							(TreeMapNode<K, V>) children.get(i), prefix
									+ (isTail ? "    " : "│   "), false));
				}
				if (children.size() >= 1) {
					builder.append(getString((TreeMapNode<K, V>) children
							.get(children.size() - 1), prefix
							+ (isTail ? "    " : "│   "), true));
				}
			}
	...
	private static class JavaCompatibleIteratorWrapper<K extends Comparable<K>, V>
		...
		public JavaCompatibleIteratorWrapper(TreeMap<K, V> map,
	...
	private static class JavaCompatibleMapEntry<K extends Comparable<K>, V>
	...
	private static class JavaCompatibleTreeMap<K extends Comparable<K>, V>
		...
		private void levelOrder(Node<K> node,
			...
			if (node.lesser != null)
				levelOrder(node.lesser, set);
			if (node.greater != null)
				levelOrder(node.greater, set);
		}
	}
}
