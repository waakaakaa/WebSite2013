...
import cn.zx.website.algorithms.data_structures.Trie.Node;
...
public class TrieMap<K extends CharSequence, V> implements Trie.INodeCreator,
	...
	private boolean validate(Trie.Node node, String string,
	 ...
	 */
	@Override
	public Trie.Node createNewNode(Trie.Node parent, Character character,
			boolean type) {
	...
	protected static class TrieMapNode<V> extends Trie.Node {
		...
		public String toString() {
			...
			if (value != null)
				builder.append("key=").append(character).append(" value=")
						.append(value).append("\n");
	...
	protected static class TrieMapPrinter {
		...
		protected static <K extends CharSequence, V> String getString(
				Trie.Node node, String prefix, String previousString,
				boolean isTail) {
			StringBuilder builder = new StringBuilder();
			String string = null;
			...
			if (node instanceof TrieMapNode) {
				...
				builder.append(prefix
						+ (isTail ? "└── " : "├── ")
						+ ((node.isWord) ? ("("
								+ String.valueOf(node.character) + ") "
								+ string + " = {" + hashNode.value + "}")
								: string) + "\n");
			}
	...
	private static class JavaCompatibleIteratorWrapper<K extends CharSequence, V>
		...
		public JavaCompatibleIteratorWrapper(TrieMap<K, V> map,
	...
	private static class JavaCompatibleTrieMap<K extends CharSequence, V>
		...
		private void levelOrder(Trie.Node node, String string,
				java.util.Set<java.util.Map.Entry<K, V>> set) {
			StringBuilder builder = new StringBuilder(string);
			TrieMapNode<V> tmn = null;
			if (node instanceof TrieMapNode) {
				tmn = (TrieMapNode<V>) node;
				if (tmn.character != Trie.Node.SENTINAL)
					builder.append(tmn.character);
				if (tmn.isWord) {
