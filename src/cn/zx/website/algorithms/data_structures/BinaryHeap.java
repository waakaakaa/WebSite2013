...
public interface BinaryHeap<T extends Comparable<T>> extends IHeap<T> {
	 ...
	 * @author Justin Wetherell <phishman3579@gmail.com>
	 */
	public static class BinaryHeapArray<T extends Comparable<T>> implements
			BinaryHeap<T> {
		...
		protected void heapUp(int nodeIndex) {
			...
			while (nodeIndex >= 0) {
				...
				if ((type == Type.MIN && parent != null && value
						.compareTo(parent) < 0)
						|| (type == Type.MAX && parent != null && value
								.compareTo(parent) > 0)) {
		...
		protected void heapDown(int index) {
			...
			int leftIndex = getLeftIndex(index);
			int rightIndex = getRightIndex(index);
			T left = (leftIndex != Integer.MIN_VALUE && leftIndex < this.size) ? this.array[leftIndex]
					: null;
			T right = (rightIndex != Integer.MIN_VALUE && rightIndex < this.size) ? this.array[rightIndex]
					: null;
			...
			if ((type == Type.MIN && left != null && right != null
					&& value.compareTo(left) > 0 && value.compareTo(right) > 0)
					|| (type == Type.MAX && left != null && right != null
							&& value.compareTo(left) < 0 && value
							.compareTo(right) < 0)) {
			...
			} else if ((type == Type.MIN && right != null && value
					.compareTo(right) > 0)
					|| (type == Type.MAX && right != null && value
							.compareTo(right) < 0)) {
				// Right is greater/lesser than node
				nodeToMove = right;
				nodeToMoveIndex = rightIndex;
			} else if ((type == Type.MIN && left != null && value
					.compareTo(left) > 0)
					|| (type == Type.MAX && left != null && value
							.compareTo(left) < 0)) {
		...
		protected static class HeapPrinter {
			...
			public static <T extends Comparable<T>> String getString(
					BinaryHeapArray<T> tree) {
			...
			private static <T extends Comparable<T>> String getString(
					BinaryHeapArray<T> tree, int index, String prefix,
					boolean isTail) {
				...
				if (leftIndex != Integer.MIN_VALUE
						|| rightIndex != Integer.MIN_VALUE) {
					children = new ArrayList<Integer>(2);
					if (leftIndex != Integer.MIN_VALUE && leftIndex < tree.size) {
						children.add(leftIndex);
					}
					if (rightIndex != Integer.MIN_VALUE
							&& rightIndex < tree.size) {
						children.add(rightIndex);
					}
				}
				if (children != null) {
					for (int i = 0; i < children.size() - 1; i++) {
						builder.append(getString(tree, children.get(i), prefix
								+ (isTail ? "    " : "│   "), false));
					}
					if (children.size() >= 1) {
						builder.append(getString(tree,
								children.get(children.size() - 1), prefix
										+ (isTail ? "    " : "│   "), true));
					}
				}
	...
	public static class BinaryHeapTree<T extends Comparable<T>> implements
			BinaryHeap<T> {
		...
		private Node<T> getNode(Node<T> startingNode, T value) {
			...
			if (startingNode != null && startingNode.value.equals(value)) {
				result = startingNode;
					...
					&& !startingNode.value.equals(value)) {
				Node<T> left = startingNode.left;
				Node<T> right = startingNode.right;
				Type type = this.type;
				if (left != null
						&& ((type == Type.MIN && left.value.compareTo(value) <= 0) || (type == Type.MAX && left.value
								.compareTo(value) >= 0))) {
				...
				if (right != null
						&& ((type == Type.MIN && right.value.compareTo(value) <= 0) || (type == Type.MAX && right.value
								.compareTo(value) >= 0))) {
		...
		protected void heapUp(Node<T> node) {
			while (node != null) {
				...
				if ((type == Type.MIN && parent != null && node.value
						.compareTo(parent.value) < 0)
						|| (type == Type.MAX && parent != null && node.value
								.compareTo(parent.value) > 0)) {
					// Node is less than parent, switch node with parent
		...
		protected void heapDown(Node<T> node) {
			...
			if ((type == Type.MIN && left != null && right != null
					&& node.value.compareTo(left.value) > 0 && node.value
				...
				if ((type == Type.MIN && right.value.compareTo(left.value) < 0)
						|| (type == Type.MAX && right.value
								.compareTo(left.value) > 0)) {
					// Right is greater/lesser than left
					nodeToMove = right;
				} else if ((type == Type.MIN && left.value
						.compareTo(right.value) < 0)
						|| (type == Type.MAX && left.value
								.compareTo(right.value) > 0)) {
					// Left is greater/lesser than right
					nodeToMove = left;
				} else {
			...
			} else if ((type == Type.MIN && right != null && node.value
					.compareTo(right.value) > 0)
					|| (type == Type.MAX && right != null && node.value
							.compareTo(right.value) < 0)) {
				// Right is greater than node
				nodeToMove = right;
			} else if ((type == Type.MIN && left != null && node.value
					.compareTo(left.value) > 0)
					|| (type == Type.MAX && left != null && node.value
							.compareTo(left.value) < 0)) {
		...
		protected static class HeapPrinter {
			...
			private static <T extends Comparable<T>> String getString(
					Node<T> node, String prefix, boolean isTail) {
				StringBuilder builder = new StringBuilder();
		...
		private static class Node<T extends Comparable<T>> {
			...
			public String toString() {
				return "value=" + value + " parent="
						+ ((parent != null) ? parent.value : "NULL") + " left="
