...
/**
 * A Patricia trie (radix tree) is a space-optimized trie data structure where
 * each non-terminating (black) node with only one child is merged with its
 * child. The result is that every internal non-terminating (black) node has at
 * least two children. Each terminating node (white) represents the end of a
 * string.
 * 
...
public class PatriciaTrie<C extends CharSequence> implements ITree<C> {
	...
	protected Node addSequence(C seq) {
		...
		for (int i = 0; i <= seq.length();) {
			...
			if (node.partOfThis(c, indexIntoParent)) {
				...
				continue;
					...
					&& indexIntoParent < node.string.length) {
		...
		if (node.string != null && indexIntoParent < node.string.length) {
			char[] parentString = Arrays.copyOfRange(node.string, 0,
					indexIntoParent);
			char[] refactorString = Arrays.copyOfRange(node.string,
					indexIntoParent, node.string.length);
			...
			if (indexIntoString < seq.length()) {
				...
				if (this.creator == null)
					...
					newParent = this.creator.createNewNode(parent,
							parentString, BLACK);
				if (parent != null)
				...
				CharSequence newString = seq.subSequence(indexIntoString,
						seq.length());
				Node newNode2 = null;
				if (this.creator == null)
					newNode2 = createNewNode(newParent, newString.toString()
							.toCharArray(), WHITE);
				else
					newNode2 = this.creator.createNewNode(newParent, newString
							.toString().toCharArray(), WHITE);
			...
			Node newNode = null;
			if (this.creator == null)
				newNode = createNewNode(node, newString.toString()
						.toCharArray(), WHITE);
			else
				newNode = this.creator.createNewNode(node, newString.toString()
						.toCharArray(), WHITE);
			...
			Node newNode = null;
			if (this.creator == null)
				newNode = createNewNode(node, seq.toString().toCharArray(),
						WHITE);
			else
				newNode = this.creator.createNewNode(node, seq.toString()
						.toCharArray(), WHITE);
			node.addChild(newNode);
			addedNode = newNode;
		}
	...
	public C remove(C seq) {
		...
		Node node = getNode(seq);
		if (node != null)
			removed = (C) (new String(node.string));
		remove(node);
		return removed;
	}
	...
	protected void remove(Node node) {
		...
		while (parent != null && parent.type == BLACK
	...
	protected Node getNode(C seq) {
		...
		if (node.string != null && indexIntoParent == (node.string.length - 1)) {
			...
			CharSequence sub = seq.subSequence(seq.length() - length,
					seq.length());
	...
	protected static class Node implements Comparable<Node> {
		...
		protected void addChild(Node node) {
			...
			if (childrenSize >= children.length) {
				children = Arrays
						.copyOf(children, (growSize + (growSize >> 1)));
			}
		...
		protected int childIndex(char character) {
			for (int i = 0; i < childrenSize; i++) {
				Node c = children[i];
				if (c.string != null && c.string.length > 0
		...
		protected boolean removeChild(int index) {
			...
			if (childrenSize >= MINIMUM_SIZE
					&& childrenSize < (shrinkSize + (shrinkSize << 1))) {
	...
	protected static class PatriciaTriePrinter<C extends CharSequence> {
		...
		protected static <C extends CharSequence> String getString(
				PatriciaTrie<C> tree) {
			return getString(tree.root, "", null, true);
		}

		protected static <C extends CharSequence> String getString(Node node,
				String prefix, String previousString, boolean isTail) {
			StringBuilder builder = new StringBuilder();
			String thisString = null;
			if (node.string != null)
				thisString = String.valueOf(node.string);
			String fullString = ((previousString != null) ? previousString : "")
					+ thisString;
			builder.append(prefix
					...
					+ ((thisString != null) ? "["
							...
							+ "] "
							+ ((node.type == WHITE) ? "(" + thisString + ") "
									+ fullString : thisString) : "["
							+ ((node.type == WHITE) ? "white" : "black") + "]")
					+ "\n");
			if (node.children != null) {
				for (int i = 0; i < node.getChildrenSize() - 1; i++) {
					builder.append(getString(node.getChild(i), prefix
							+ (isTail ? "    " : "│   "), thisString, false));
				}
				if (node.getChildrenSize() >= 1) {
					builder.append(getString(
							node.getChild(node.getChildrenSize() - 1), prefix
									+ (isTail ? "    " : "│   "), thisString,
							true));
				}
			}
	...
	public static class JavaCompatiblePatriciaTrie<C extends CharSequence>
			extends java.util.AbstractCollection<C> {
		 ...
		 * {@inheritDoc}
		 * 
		 * WARNING: This iterator makes a copy of the trie's contents during
		 * it's construction!
		 */
		@Override
		...
		private static class PatriciaTrieIterator<C extends CharSequence>
			...
			private void getNodesWhichRepresentsWords(PatriciaTrie.Node node,
					String string,
					java.util.Map<PatriciaTrie.Node, String> nodesMap) {
				StringBuilder builder = new StringBuilder(string);
				if (node.string != null)
					builder.append(node.string);
				...
				for (int i = 0; i < node.childrenSize; i++) {
					...
					getNodesWhichRepresentsWords(child, builder.toString(),
							nodesMap);
				}
			}
			...
			public C next() {
				...
				java.util.Map.Entry<PatriciaTrie.Node, String> entry = iterator
						.next();
